...
async def require_bearer(
    ...
    async with aiosqlite.connect(db_path) as db:
        cur = await db.execute("SELECT id FROM devices WHERE token = ?", (token,))
...
async def pair(body: PairRequest) -> dict[str, Any]:
    ...
    async with aiosqlite.connect(db_path) as db:
        await db.execute(
            "INSERT INTO devices (id, name, pairing_code, token, created_at) VALUES (?, ?, ?, ?, ?)",
            (device_id, body.device_name, body.pairing_code, token, utc_now()),
        )
        await db.commit()
    return {"deviceId": device_id, "token": token, "agentUrl": f"http://{HOST}:{PORT}"}
...
def _run_analyze_sync(body: AnalyzeRequest, path_str: str, fingerprint: str, metadata_json: str) -> dict[str, Any]:
    ...
    racket = track_racket(pose_frames=pose["frames"], fps=fps)
    try:
...
async def analyze(
    ...
    try:
        result = await asyncio.to_thread(_run_analyze_sync, body, path_str, fingerprint, metadata_json)
    except MediaError as e:
        raise HTTPException(400, str(e)) from e
    ...
    return {
        ...
        "agentMediaUrl": f"http://{HOST}:{PORT}/media/{body.capture_id}",
    }
...
async def metrics_series(metric_id: str, _token: str = Depends(require_bearer)) -> dict[str, Any]:
    db_path = get_db_path(DATA_DIR)
    async with aiosqlite.connect(db_path) as db:
        cur = await db.execute(
            "SELECT id, summary_json, created_at FROM analysis_runs ORDER BY created_at ASC"
        )
        rows = await cur.fetchall()
    points = []
