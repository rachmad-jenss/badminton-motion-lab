...
1. Run web lint/typecheck/build, contracts tests, agent pytest/smoke/lifecycle,
   no-synthetic checks, readiness integrity, and the full Playwright suite.
2. Start the website and exercise the critical browser flow directly against
   the running local site, recording the exact result and preserving unrelated
...
- PR URL, commit list, review/comment state, CI runs, post-merge `main` run,
  direct website test result, and branch cleanup result.
